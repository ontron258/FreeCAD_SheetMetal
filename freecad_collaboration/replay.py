...
def _is_protected_property(obj, property_name):
    try:
        status = {str(value) for value in obj.getPropertyStatus(property_name)}
    except Exception:
        return False
    return bool(status & {"Immutable", "NoModify"})


def _restore_expression_engine(obj, structured_value):
    for path, _expression in list(obj.ExpressionEngine):
        obj.setExpression(path, None)
    for path, expression in structured_value.get("expressions", []):
        ...
        try:
            ...
            for operation in packet.operations:
                ...
                if operation.kind == "add_property":
                    ...
                    # Some workbenches report derived immutable properties in
                    # the same transaction as the user input. They must be
                    # regenerated by recompute, not assigned on the peer.
                    if _is_protected_property(obj, operation.property_name):
                        continue
                    if (
                        isinstance(operation.structured_value, dict)
                        and operation.structured_value.get("kind")
                        == "expression_engine"
                    ):
                        _restore_expression_engine(obj, operation.structured_value)
                    elif operation.structured_value is not None:
            ...
            for operation in packet.operations:
                ...
                try:
                    ...
                    # A preceding input change can make a feature proxy remove
                    # the same generated child before the explicit historical
                    # delete operation is reached. Deletion is idempotent.
                    continue
